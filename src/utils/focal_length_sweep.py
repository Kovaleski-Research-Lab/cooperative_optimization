...
if __name__ == "__main__":
    ...
    ideal_focal_length = config['modulators'][1]['focal_length']
    # Create a range of focal lengths around the ideal focal length
    # +- 10% of the ideal focal length
    focal_lengths = np.linspace(ideal_focal_length * 0.9, ideal_focal_length * 1.1, 25)
    fig, ax = plt.subplots(1, 1)
    im0 = ax.imshow(np.zeros((1080, 1920)))
    for focal_length in focal_lengths:
        # Set the focal length in the config
        ...
        min_val = np.min(camera_image)
        max_val = np.max(camera_image)
