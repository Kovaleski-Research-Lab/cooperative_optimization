...
sys.path.append('../')
from models.models import CooperativeOpticalModelRemote, CooperativeOpticalModelRemoteSim2Real
...
if __name__ == "__main__":
    ...
    #model = CooperativeOpticalModelRemote.load_from_checkpoint('../../results/coop_MNIST_bench_image/version_0/checkpoints/model-epoch=14.ckpt', params=config)
    ...
    with torch.no_grad():
        for j, d in enumerate(tqdm(data, desc='Running data', leave=False)):
            try:
                ...
                simulation_image = outputs['simulation_outputs']['images'].squeeze().detach().cpu()
    ...
    torch.save(output, '../../results/coop_MNIST_bench_image/version_0/baseline_eval_valid.pt')
